...
const (
	CreateNFTClass   = "/v2/nft/classes"                // 创建 NFT 类别
	QueryNFTClasses  = "/v2/nft/classes"                // 查询 NFT 类别
	QueryNFTClass    = "/v2/nft/classes/%s"             // 查询 NFT 类别详情
	TransferNFTClass = "/v2/nft/class-transfers/%s/%s"  // 转让 NFT 类别
	MintNFT          = "/v2/nft/nfts/%s"                // 发行 NFT
	TransferNFT      = "/v2/nft/nft-transfers/%s/%s/%s" // 转让 NFT
	EditNFT          = "/v2/nft/nfts/%s/%s/%s"          // 编辑 NFT
	BurnNFT          = "/v2/nft/nfts/%s/%s/%s"          // 销毁 NFT
	QueryNFTs        = "/v2/nft/nfts"                   // 查询 NFT
	QueryNFT         = "/v2/nft/nfts/%s/%s"             // 查询 NFT 详情
	QueryNFTHistory  = "/v2/nft/nfts/%s/%s/history"     // 查询 NFT 历史记录
)
...
type QueryNFTClassRes struct {
	Data struct {
		...
		EditableByClassOwner int    `json:"editable_by_class_owner"` //NFT 类别权限的控制功能， 此 NFT 类别的权属者可以编辑这个类别下所有的 NFT ：1可编辑 0不可编辑
		TxHash               string `json:"tx_hash"`                 // 创建 NFT 类别的 Tx Hash
		TimeStamp            string `json:"timestamp"`               // 创建 NFT 类别的时间戳（UTC 时间）
	} `json:"data"`
}
...
type QueryNFTHistoryRes struct {
	Data struct {
		...
		OperationRecords []struct {
			...
			Operation int    `json:"operation"` // NFT 操作类型；Enum: "mint" "edit" "transfer" "burn"
